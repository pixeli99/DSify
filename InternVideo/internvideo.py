...
__all__ = ["load_model", "load_video", "load_images", "tokenize"]
...
def load_video(path):
    ...
    video = video_reader.get_batch(np.linspace(0, video_len - 1, 8).astype(np.int32)).byte()
...
def load_images(path):
    ...
    down_idx = np.linspace(0, 15, 8).astype(np.int)
    for idx in down_idx:
